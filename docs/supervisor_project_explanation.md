...
Each selected response is tokenized and passed through the locally stored
Llama-3.1-8B model. A response is not separately fed into every layer. It passes
...
- 14,656,471,040 verified finite activation values, occupying 27.31 GiB.
...
selected layer, using the same examples at each layer. Conversation-grouped splits
will prevent responses from the same transcript or dialogue appearing in both
training and test sets. Accuracy and macro-F1 will be reported alongside shuffled-
label baselines. Probing shows where information is represented; it does not prove
that the model causally uses that information.
...
The 200+200 activation-capture stage is complete. Linear probing, activation
...
been implemented and run. Grouped split ratios, the probe token aggregation,
patching corruption construction and logit definition, and a compatible SAE source
still require explicit methodological decisions.
